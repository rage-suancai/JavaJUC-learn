 ...
 *                  AQS ---newCondition()--> ConditionObject
 *                      ---newCondition()--> ConditionObject
 *                                            [firstWaite] -----------> Node                  Node                  Node
 *                                            [lastWaiter]          [nextWaiter] -------> [nextWaiter] -------> [nextWaiter]
 *                                                 |                                                                 |
 *                                                 |----------------------------------------------------------------->
 *
 ...
 *                      Node node = addConditionWaiter(); // 为当前线程创建一个新的结点 并将其加入到条件队列中
 ...
 *                          unlinkCancelledWaiters(); // 将等待队列中 不是Node.CONDITION状态的结点移除
 *                      if (interruptMode != 0) // 依然是响应中断
 *                          reportInterruptAfterWait(interruptMode);
 *                      // OK 接着该干嘛干嘛
 *                  }
 *
 * 实际上 await()方法比较中规中矩 大部分操作也在我们的意料之中 那么我们接着来看signal()方法是如何实现的 同样的 为了防止各位绕晕 先明确signal的目标:
 *
 *       > 只有持有锁的线程才能唤醒锁所属的Condition等待的线程
 *       > 优先唤醒条件队列中的当第一个 如果唤醒过程中出现问题 接着往下找 直到找到一个可以唤醒的
 ...
 *                      if (fist != null) // 如果队列不为空 获取到了 那么就可以开始唤醒操作
 *                          doSignal(first);
 *                  }
 *
 *                  private void doSignal(Node first) {
 *                      do {
 *                          if ((firstWaiter = first.nextWaiter) == null) // 如果当前结点在本轮循环没有后继结点了 条件队列就为空了
 *                              lastWaiter = null; // 所以这里相对于是直接清空
 *                          first.nextWaiter = null; // 将给定结点的下一个结点设置为null 因为当前结点马上就会离开条件队列了
 *                      } while (!transferForSignal() && // 接着往下看
 *                               (first = firstWaiter) != null); // 能走到这里只能说明给定结点被设定为了取消状态 那就继续看下一个结点
 *                  }
 *
 *                  final boolean transferForSignal(Node node) {
 *                      // 如果这里CAS失败 那有可能此结点被设定为了取消状态
 *                      if (!compareAndSetWaitStatus(node, Node.CONDITION, 0))
 *                          return false;
 *
 *                      // CAS成功之后 结点的等待状态就变成了默认值0 接通过enq方法直接将结点丢进AQS的等待队列中 相当于唤醒并且可以等待获取锁了
 *                      // 这里enq方法返回的是加入之后等待队列队尾的驱结点 就是原来的tail
 *                      Node p = enq(node);
 *                      int ws = p.waitStatus; // 保存前驱结点的等待状态
 *                      // 如果上一个结点的状态为取消 或者尝试设置上一个结点的状态为SIGNAL失败(可能是在ws>0判断之后马上变成了取消状态 导致CAS失败)
 *                      if (ws > 0 || !compareAndSetWaitStatus(p, ws, Node.SIGNAL))
 *                          LockSupport.unpark(node.thread); // 直接唤醒线程
 *                      return true;
 *                  }
 *
 * 其实最后让人不理解的就是倒数第二行 明明上面都正常进入到AQS等待队列了 应该是可以开始走正常流程了 那么这里为什么还要提前来一次unpark呢?
 *
 * 这里其实是为了进行优化而编写 直接unpark会有两种情况:
 *
 *      > 如果插入结点前 AQS等待队列的队尾结点就已经被取消 则满足wc > 0
 *      > 如果插入node后 AQS内部等待队列的队尾结点已经稳定 满足tail.waitStatus == 0 但在执行ws > 0之后!compareAndSetWaitStatus(p,ws,Node.SIGNAL)之前被取消
 *        则CAS也会失败 满足compareAndSetWaitStatus(p,ws,Node.SIGNAL) == false
 *
 * 如果这里被提前unpark 那么在await()方法中将可以被直接唤醒 并跳出while循环 直接开始争抢锁 因为前一个等待结点是被取消的状态 没有必要再等它了
 *
 * 所以 大致流程如下:
 *
 *      https://img-blog.csdnimg.cn/img_convert/45dbe6e9d7c0fca94af1d96afd812f7a.png
 *
 * 只要把整个流程理清楚 还是很好理解的
 */
public class Main {
