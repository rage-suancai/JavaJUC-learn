package MultithreadProgrammingCore.concurrentContainer;
 ...
 * 在之前我们可以使用Vector或是Hashtable来解决 但是它们的效率实在是太低了 完全依靠锁来解决问题 因此现在已经很少使用它们了 这里也不会再去进行解释
 *
 ...
 *                      Runnable r = () -> {
 *                          for (int i = 0; i < 100; i++) list.add("Hello World");
 *                      };
 ...
 *                  }
 *
 * 我们发现 使用了CopyOnWriterArrayList之后 再没出现过上面的问题
 *
 ...
 *                  }
 *
 * 可以看到添加操作是直接上锁 并且会先拷贝一分当前存放元素的数组 然后对数组进行修改 再将此数组替换(CopyOnWrite) 接着我们来看读操作:
 *
 *                  public E get(int index) {
 *                      return get(getArray(), index);
 *                  }
 *
 * 因此 CopyOnWriteArrayList对于读操作不加上锁 而对于写操作是加锁的 类似于我们前面讲解的读写锁机制 这样就可以保证不丢失读性能的情况下 写操作不会出现问题
 *
 ...
 *                          int finalI = i;
 *                          new Thread(() -> {
 *                              for (int j = 0; j < 100; j++) map.put(finalI * 100 + j, "Hello World");
 *                          }).start();
 *                      }
 *                      try {
 *                          TimeUnit.SECONDS.sleep(1);
 ...
 * 那我就干脆多搞几把锁 让你们每个人都能拿到 这样就不会存在等待的问题了 而JDK7之前 ConcurrentHashMap的原理也比较类似 它将所有数据分为一段一段地存储
 * 先分很多段出来 每一段都给一把锁 当一个线程占锁访问时 只会占用其中一把锁 也就是仅仅锁了一小段数据 而其他段的数据依然可以被其他线程正常访问
 *
 *      https://img-blog.csdnimg.cn/img_convert/aeda7fe6747a70307f6e69cbd4bb59e5.png
 *
 * 这里我们重点讲解JDK8之后它是怎么实现的 它采用了CAS算法配合锁机制实现 我们先来回顾一下JDK8下的HashMap是什么样的结构:
 *
 *      https://img-blog.csdnimg.cn/img_convert/3ad05990ed9e29801b1992030c030a00.png
 *
 * HashMap就是利用了哈希表 哈希表的本质其实就是应该用于存放后续节点的头结点的数组 数组里面的每一个元素都是一个头结点(也可以说就是一个链表) 当要新插入一个数据时
 * 会先计算该数据的哈希值 找到数组下标 然后创建一个新的节点 添加到对应的链表后面 当链表的长度达到8时 会自动将链表转换为红黑树
 * 这样能使用原有的查询效率大幅度降低 当使用红黑树之后 我们就可以利用二分搜索思想 快速地去寻找我们想要的结果 而不是像链表一样挨个去看
 *
 * 又是基础不牢地动山摇环节 由于ConcurrentHashMap的源码比较复杂 所以我们先从最简单的构造方法开始下手:
 *
 *      https://img-blog.csdnimg.cn/img_convert/5d19b70a4e0a851cacc364efb4071924.png
 *
 ...
 *
 *      https://img-blog.csdnimg.cn/img_convert/f1beb686066cbcec5d61dae9b9780fb5.png
 *
 * 我们接着来看看get()操作:
 *
 ...
 *
 * 其实这里也只要简单地介绍了一下它的运行机制 ConcurrentHashMap真正的难点在于扩容和迁移操作
 * 我们主要了解的是他的并发执行机制 有关它的其他实现细节 这里暂时不进行讲解
 */
public class Main {
