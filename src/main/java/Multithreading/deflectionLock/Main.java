 ...
 *
 * 可以从之前的Mark Word结构中看到 偏向锁也会通过CAS操作记录线程的ID 如果一直都是同一个线程获取此锁 那么完全没有必要再进行额外的CAS操作
 * 当然 如果有其他线程来抢了 那么偏向锁会根据当前状态 决定是否要恢复到未锁定或是膨胀为轻量级
 *
 ...
 * 而偏向锁的Mark Word数据结构无法保存hash值 如果对象已经是偏向锁状态 再去调用hashCode()方法 那么会直接将锁升级为重量级锁 并将哈希值存放在monitor(有预留位置保存)中
 *
 *      https://img-blog.csdnimg.cn/img_convert/791c12dd14274fe6d2b4fd7f3be23354.png
 *
 * 锁消除和锁粗化
